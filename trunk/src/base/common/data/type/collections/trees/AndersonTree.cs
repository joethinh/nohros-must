...
namespace Nohros.Data.Collections
{
      ...
      IEnumerable
  {
    ...
    /// <summary>
    /// Initializes a new empty <see cref="AndersonTree&alt;TKey, TValue&gt;"/>
    /// class.
    /// </summary>
    /// <param name="comparer">The comparer to use when comparing items</param>
    /// <exception cref="ArgumentNullException"><paramref name="comparer"/> is
    /// null</exception>
    ...
    /// <see cref="AndersonTree&alt;TKey, TValue&gt;"/> class by using the
    /// specified <see cref="System.Comparison&alt;TKeyt&gt;"/> delegate.
    /// </summary>
    /// <param name="comparison">The <see cref="Comparison&alt;T&gt;"/> to use
    ...
    /// <summary>
    /// Retieves a <see cref="AndersonTreeNode"/> object in the
    /// <see cref="AndersonTree"/> associated with the specified key.
    /// </summary>
    ...
    /// key is found; otherwise an AndersonTreeNode with level zero - sentinel
    /// node.</returns>
    ...
    internal AndersonTreeNode<TKey, TValue> FindNode(TKey key) {
      ...
      int cmp;
      for (AndersonTreeNode<TKey, TValue> node = _root;
        node.Level != 0; node = (cmp < 0) ? node.Left : node.Right) {
    ...
    /// node in the tree.</param>
    /// <remarks>The <paramref name="action"/> must return true in order to
    /// allow the traversal to continue.</remarks>
      ...
      if (_root.Level != 0) {
        Stack<AndersonTreeNode<TKey, TValue>> stack =
          new Stack<AndersonTreeNode<TKey, TValue>>
            (2 * ((int)Math.Log((double)(_count + 1))));
    ...
    /// <param name="add">a value indicating when the item will be added or
    /// modified</param>
    private void Insert(TKey key, TValue value, bool add) {
      ...
      List<AndersonTreeNode<TKey, TValue>> path =
        new List<AndersonTreeNode<TKey, TValue>>(_count);
      ...
      // create the new node
      node._childs[dir] =
        new AndersonTreeNode<TKey, TValue>(key, value, sentinel);
    ...
    /// <see cref="AndersonTree&lt;TKey,TValue&gt;"/>
    /// </summary>
    /// <param name="keyValuePair">The value to locate in the
    ...
    /// <see cref="AndersonTree&lt;TKey,TValue&gt;"/> to an
    /// <see cref="Array"/>, starting at a particular <see cref="Array"/>
    /// index.
    /// </summary>
    /// <param name="array">The one-dimensional <see cref="Array"/> that is
    /// the destination of the elements copied form the
    /// <see cref="AndersonTree&lt;TKey,TValue&gt;"/> The array must have
    ...
    /// destination array.-or-ValueType
    /// <see cref=" KeyValuePair&lt;TKey, TValue&gt;"/> cannot be cast
    ...
    /// already exists in the AndersonTree<,></exception>
    /// <exception cref="ArgumentNullException">key is null</exception>
    ...
    /// <returns>true if the element is successfully removed; otherwise,
    /// false. This method also returns false if key was not found in the
    /// tree</returns>
    /// <exception cref="ArgumentNullException">key is null</exception>
    ...
    /// specified key is not found, attempting to get it throws a
    /// <see cref="KeyNotFoundException"/>, and attempting to set it creates a
    /// new element with the specified key</returns>
    /// <exception cref="ArgumentNullException">key is null</exception>
    /// <exception cref="KeyNotFoundException">The key does not exists in the
    /// tree.</exception>
