...
{
  /// <summary>
  /// A <see cref="YQueue{T}"/> that additionally supports operations taht wait
  ...
  /// </summary>
  /// <remarks>
  /// The queue does blocks when the number of elements in the queue is equals
  /// to (alpha * granularity).
  /// </remarks>
  public class YBlockingQueue<T>
  {
    class Chunk
    {
    ...
    readonly AutoResetEvent signaler_;
    volatile Chunk tail_chunk_;

    #region .ctor
    /// <summary>
    /// Initializes a new instance of the <see cref="YBlockingQueue{T}"/> class
    /// that has the default capacity(32) and the specified upper-bound
    /// </summary>
    /// <param name="alpha">
    /// The maximum number of block of elements(alpha * granularity) that
    /// should be created before the <see cref="Enqueue"/> method blocks,
    /// waiting for elements to be consumed.
    /// </param>
    public YBlockingQueue(int alpha)
      : this(kDefaultCapacity, alpha) {
    }
    ...
    /// waiting for elements to be consumed.
    /// </param>
    public YBlockingQueue(int alpha, int granularity) {
      if (granularity < 0) {
        throw new ArgumentOutOfRangeException(string.Format(
          Resources.Resources.ArgumentOutOfRange_arg_NonNegNum, "granularity"));
      }
    ...
    public void Enqueue(T element) {
      Enqueue(element, Timeout.Infinite);
    }
    ...
    /// The element to be added to the back end of the queue.
    /// </param>
    public void Enqueue(T element, int timeout) {
      ...
      // If the tail chunk is full, checks if the maximum number of chunks
      // has been reached.
      if (tail_pos == granularity_) {
        if ((tail_chunk_.distance - divider_.distance) >= alpha_) {
          if (!signaler_.WaitOne(timeout)) {
            return;
          }
        }
      }
    ...
    /// <summary>
    /// Removes all elements from the <see cref="YBlockingQueue{T}"/>.
    /// </summary>
    /// <remarks>
    /// <para>
    ...
    /// removed from the beginning of the <see cref="YBlockingQueue&lt;T&gt;"/>, if
    ...
    /// <summary>
    /// Gets a value indicating whether the <see cref="YBlockingQueue{T}"/> is empty.
    /// </summary>
    /// <remarks>
