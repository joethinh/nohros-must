  ...
  public class MergeHttpHandler: IHttpHandler
  {
    ...
    void GetMergedContent(string content_group_name, string mime_type) {
      ...
      if (cached_file != null) {
        ...
        if (cached_file == null) {
          ...
          FileLogger.ForCurrentProcess.Error("[GetMergedContent   Nohros.Net.MergeHttpHandler]   The files to merge are not found.");
          context_.Response.StatusCode = kResourceNotFoundCode;
          context_.Response.End();
        }
      }
    ...
    public void ProcessRequest(HttpContext context) {
      ...
      if (position == -1) {
        ...
        FileLogger.ForCurrentProcess.Error("[ProcessRequest   Nohros.Net.MergeHttpHandler] The handler is configured incorrectly. Check handlers session of the the web.config file");
        context_.Response.StatusCode = kResourceNotFoundCode;
        context_.Response.End();
        return;
      }
