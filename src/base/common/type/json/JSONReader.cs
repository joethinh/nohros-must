...
namespace Nohros
{
  ...
  public class JSONReader
  {
    ...
    bool ReadInt(Token token, bool can_have_leading_zeros) {
      ...
      // Read in more digits
      c = first;
      while (pos < json_length_ && char.IsDigit(c)) {
        ++token.length;
        ++len;

        // early terminate the loop when we have no more chars to read.
        if (++pos >= json_length_) {
          break;
        }
        c = json_[pos];
    ...
    /// </remarks>
    public IValue JsonToValue(string json, bool check_root, bool allow_trailing_comma) {
      json_ = json;
      ...
      IValue root = BuildValue(check_root);
      if (root != null) {
    ...
    IValue DecodeNumber(Token token) {
    ...
    IValue DecodeString(Token token) {
    ...
    bool NextStringMatch(string str) {
      return string.Compare(json_, json_pos_, str, 0, str.Length) == 0;
    }
